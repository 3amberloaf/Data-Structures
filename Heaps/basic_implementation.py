class MaxHeap: 
	...
	# Constructor function to initialize the new instances
	def __init__(self, maxSize): 
		self.maxSize = maxSize 
		self.arr = [None]*maxSize # sets the maxSize to None
		self.heapSize = 0 # indicates the heap is currently empty
	...
	def MaxHeapify(self, i): 
		...
		r = self.rChild(i) 
		largest = i 
		if l < self.heapSize and self.arr[l] > self.arr[i]: # if left child node is greater than current node, it is swapped to the largest
			largest = l 
		if r < self.heapSize and self.arr[r] > self.arr[largest]: # if right child node is greater than current node, it is swapped to the largest
			largest = r 
		if largest != i: # if the largest node is not i, swap the larger node up
			temp = self.arr[i] 
			self.arr[i] = self.arr[largest] 
			self.arr[largest] = temp 
			self.MaxHeapify(largest) # restarts
